  ...
  // Use this for initialization
	...
	void Update () {
		if (!control_state.jump)
			control_state.jump = InputManager.GetButtonDown ("Jump");
		if (control_state.heightChange == 0)
			control_state.heightChange = (InputManager.GetButtonDown ("HeightUp") ? 1 : 0)
        - (InputManager.GetButtonDown("HeightDown") ? 1 : 0);
	}
